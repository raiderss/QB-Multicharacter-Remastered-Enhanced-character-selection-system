...
local randomAnimations = {
    {"mp_player_inteat@burger", "mp_player_int_eat_burger"},
    ...
    {"anim@mp_player_intcelebrationmale@thumb_on_ears", "thumb_on_ears"},
...
local function playRandomAnimation()
    local randomAnim = randomAnimations[math.random(#randomAnimations)]
    RequestAnimDict(randomAnim[1])
    while not HasAnimDictLoaded(randomAnim[1]) do
        Wait(0)
    end
    TaskPlayAnim(charPed, randomAnim[1], randomAnim[2], 8.0, -8.0, -1, 0, 0, false, false, false)
    RemoveAnimDict(randomAnim[1])
end
...
RegisterNUICallback('selectCharacter', function(data, cb)
    ...
    DoScreenFadeOut(10)
    playRandomAnimation()
    Wait(1500) -- Animasyonun görünmesi için kısa bir bekleme
    TriggerServerEvent('qb-multicharacter:server:loadUserData', cData)
    openCharMenu(false)
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
...
RegisterNUICallback('cDataPed', function(nData, cb)
    ...
    if cData ~= nil then
        ...
        if model ~= nil then
            ...
            Wait(500)
            playRandomAnimation() -- Her karakter değişiminde rastgele animasyon
        else
            initializePedModel()
        end
        cb('ok')
